			...
			<nav>
				...
				<ul class="nav nav-pills float-right">
					...
					<li class="nav-item">
						...
						<a class="nav-link active" href="preventivi-list.php">Preventivi <span class="sr-only">(current)</span></a>

					</li>

					<li class="nav-item">

						<a class="nav-link" href="clienti-list.php">Clienti</a>
...
error_reporting(-1);
						...
						foreach($conn->query("SELECT * FROM ac_preventivi_elementi WHERE preventivo_id = $preventivo_id ORDER BY item_order") as $elemento) {

							$imgurl = $conn->query("SELECT image_url FROM ac_frutti WHERE frutto_id = ".$elemento['frutto_id'])->fetchColumn();
			...
			<div class="row marketing">
				...
				<div class="col-6">
					...
					<?php $sql = 'SELECT frutto_id, denominazione, prezzo, image_url FROM ac_frutti WHERE tipo_id = 1 ORDER BY denominazione';

    					foreach ($conn->query($sql) as $presa) {

							echo("<a href=\"/conf-mngr.php?preventivo_id=".$preventivo_id."&type=add&modulo_id=".$presa['frutto_id']."\"><figure class=\"figure\"><img alt=\"".$presa['denominazione']." (".$presa['prezzo'].")\" src=\"".$presa['image_url']."\" style=\"max-height: 90px; max-height: 90px; padding: 10px 10px 10px 10px;\"><figcaption class=\"figure-caption\"><small>".$presa['denominazione']."<br> &euro;".$presa['prezzo']."</small></figcaption></figure></a>");
				...
				<div class="col-6">
					...
					<div class="row">
						...
						<?php $sql = 'SELECT frutto_id, denominazione, prezzo, image_url FROM ac_frutti WHERE tipo_id = 2 ORDER BY denominazione';
					...
					<div class="row">
						...
						<?php $sql = 'SELECT frutto_id, denominazione, prezzo, image_url FROM ac_frutti WHERE tipo_id = 3 ORDER BY denominazione';
